...
void vk_generate_image_upload_data( image_t *image, byte *data, Image_Upload_Data *upload_data ) {
	...
	upload_data->buffer = (byte*)R_Malloc(2 * 4 * scaled_width * scaled_height, TAG_TEMP_WORKSPACE, qfalse);
	if (data == NULL) {
		Com_Memset(upload_data->buffer, 0, 2 * 4 * scaled_width * scaled_height);
	}

	if ((scaled_width != width || scaled_height != height) && data) {
		resampled_buffer = (byte*)R_Malloc(scaled_width * scaled_height * 4, TAG_TEMP_WORKSPACE, qfalse);
	...
	scaled_buffer = (unsigned int*)R_Malloc(sizeof(unsigned) * scaled_width * scaled_height, TAG_TEMP_WORKSPACE, qfalse);
...
byte *vk_resample_image_data( const image_t *image, byte *data, const int data_size, int *bytes_per_pixel )
{
	...
	case VK_FORMAT_B4G4R4A4_UNORM_PACK16:
		buffer = (byte*)R_Malloc(data_size / 2, TAG_TEMP_WORKSPACE, qfalse);
		p = (uint16_t*)buffer;
	...
	case VK_FORMAT_B8G8R8A8_UNORM:
		buffer = (byte*)R_Malloc(data_size, TAG_TEMP_WORKSPACE, qfalse);
	...
	case VK_FORMAT_R8G8B8_UNORM: {
		buffer = (byte*)R_Malloc( (data_size * 3) / 4, TAG_TEMP_WORKSPACE, qfalse);
...
image_t *R_CreateImage( const char *name, byte *pic, int width, int height, imgFlags_t flags ){
    ...
    //image = (image_t*)R_Malloc(sizeof(*image) + namelen + namelen2, TAG_IMAGE_T, qtrue);
