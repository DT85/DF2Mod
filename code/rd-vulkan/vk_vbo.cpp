...
void R_BuildWorldVBO(msurface_t *surf, int surfCount)
{
	...
	// 0 item is unused
	vbo->items = (vbo_item_t*)R_Hunk_Alloc((numStaticSurfaces + 1) * sizeof(vbo_item_t), qtrue);
	vbo->items_count = numStaticSurfaces;

	// last item will be used for run length termination
	vbo->items_queue = (int*)R_Hunk_Alloc((numStaticSurfaces + 1) * sizeof(int), qtrue);
	...
	// vertex buffer
	vbo_size += ibo_size;
	vbo->vbo_buffer = (byte*)R_Malloc(vbo_size, TAG_TEMP_WORKSPACE, qfalse);
	vbo->vbo_offset = 0;
	vbo->vbo_size = vbo_size;

	// index buffer
	vbo->ibo_buffer = (byte*)R_Hunk_Alloc(ibo_size, qtrue);
	vbo->ibo_offset = 0;
	vbo->ibo_size = ibo_size;

	// ibo runs buffer
	vbo->ibo_items = (ibo_item_t*)R_Hunk_Alloc(((numStaticIndexes / MIN_IBO_RUN) + 1) * sizeof(ibo_item_t), qtrue);
	vbo->ibo_items_count = 0;

	surfList = (msurface_t**)R_Malloc(numStaticSurfaces * sizeof(msurface_t*), TAG_TEMP_WORKSPACE, qfalse);
