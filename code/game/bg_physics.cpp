...
static qboolean LoadMapData ( const char* mapname, char** mapBuffer )
{
    ...
    fileLen = gi.FS_FOpenFile (filename, &f, FS_READ);
    if ( fileLen == -1 || !f )
    {
        gi.FS_FCloseFile (f);
    ...
    gi.Malloc(sizeof (char) * fileLen, TAG_TEMP_WORKSPACE, qfalse);
    
    gi.FS_Read (*mapBuffer, fileLen, f);
    gi.FS_FCloseFile (f);
...
{   
    ...
    if ( !LoadSurfaces ((byte*)mapBuffer, &header->lumps[LUMP_SURFACES], &header->lumps[LUMP_DRAWVERTS], &header->lumps[LUMP_MODELS], &header->lumps[LUMP_DRAWINDEXES], &header->lumps[LUMP_SHADERS]) )
    {
		...
		gi.Free(reinterpret_cast<void**>(&mapBuffer));
        return qfalse;
    }
