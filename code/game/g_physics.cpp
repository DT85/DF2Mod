...
static void G_PhysicsEntityThink ( const NewtonBody* body, dFloat timestep, int threadIndex )
{
    ...
    gi.trace (&tr, ent->currentOrigin, ent->mins, ent->maxs, ent->currentOrigin, ent->s.number, CONTENTS_WATER, (EG2_Collision)0, 0);
    if ( tr.fraction != 1.0f )
    {
...
static void G_PhysicsEntitySetTransform ( const NewtonBody* body, const dFloat* matrix, int threadIndex )
{
    ...
    VectorCopy (newPosition, ent->currentOrigin);
...
void G_AddStaticEntity ( gentity_t* ent, entityType_t entityType )
{
    ...
    dMatrix matrix (GetIdentityMatrix());
    BG_QuakeToSIUnits2 (ent->currentOrigin, matrix.m_posit);
...
void G_UpdateStaticBodies()
{
            ...
            ++it )
    {
        ...
        BG_QuakeToSIUnits2 (sbody.ent->currentOrigin, matrix.m_posit);
